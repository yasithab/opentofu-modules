...
variable "repl_instance_dns_name_servers" {
  description = "Custom DNS name servers separated by a space for the replication instance"
  type        = string
  default     = null
}

variable "repl_instance_kerberos_authentication_settings" {
  description = "Kerberos authentication settings for the replication instance. Includes key_cache_secret_iam_arn, key_cache_secret_id, and krb5_file_contents"
  type        = any
  default     = null
}
...
variable "endpoint_timeouts" {
  description = "A map of timeouts for endpoint create/delete operations"
  type        = map(string)
  default     = {}
}
