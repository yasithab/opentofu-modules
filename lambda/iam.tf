locals {
  create_role = local.enabled && var.create_function && !var.create_layer && var.create_role
  ...
  log_group_arn_regional = try(data.aws_cloudwatch_log_group.lambda[0].arn, aws_cloudwatch_log_group.lambda.arn, "")
  log_group_name         = try(data.aws_cloudwatch_log_group.lambda[0].name, aws_cloudwatch_log_group.lambda.name, "")
...
resource "aws_iam_policy" "logs" {
  ...
  lifecycle {
    enabled = local.create_role && var.attach_cloudwatch_logs_policy
  }
}
...
resource "aws_iam_policy" "dead_letter" {
  ...
  lifecycle {
    enabled = local.create_role && var.attach_dead_letter_policy
  }
}
...
resource "aws_iam_policy" "vpc" {
  ...
  lifecycle {
    enabled = local.create_role && var.attach_network_policy
  }
}
...
resource "aws_iam_policy" "async" {
  ...
  lifecycle {
    enabled = local.create_role && var.attach_async_event_policy
  }
}

resource "aws_iam_role_policy_attachment" "async" {
  role       = aws_iam_role.lambda.name
  policy_arn = aws_iam_policy.async.arn
