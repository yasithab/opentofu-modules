...
variable "provisioned_concurrency_skip_destroy" {
  description = "Whether to retain the provisioned concurrency configuration upon destruction. Defaults to false. Useful when you want to destroy the function but keep the concurrency config (e.g. to avoid cold starts on re-creation)."
  type        = bool
  default     = false
}
...
variable "source_kms_key_arn" {
  description = "ARN of the KMS key used to encrypt the function's deployment package"
  type        = string
  default     = null
}

variable "tenancy_config" {
  description = "Configuration for Lambda function tenancy. Supports mode attribute"
  type        = any
  default     = null
}

variable "capacity_provider_config" {
  description = "Configuration block for Lambda Capacity Provider. Contains lambda_managed_instances_capacity_provider_config sub-block."
  type        = any
  default     = null
}

variable "durable_config" {
  description = "Configuration block for durable function settings. Requires execution_timeout; optionally accepts retention_period."
  type        = any
  default     = null
}

variable "code_sha256" {
  description = "Base64-encoded representation of the source code package file. Used to trigger updates."
  type        = string
  default     = null
}

variable "publish_to" {
  description = "Whether to publish to an alias or version number. Omit for regular version publishing. Option is LATEST_PUBLISHED."
  type        = string
  default     = null
}
