...
variable "encryption_configuration" {
  description = "Encryption configuration block for the ECR repository. Supports encryption_type (AES256 or KMS) and kms_key (ARN)."
  type        = any
  default     = null
}

variable "image_tag_mutability_exclusion_filter" {
  description = "Configuration block for tag mutability exclusion filter. Required when image_tag_mutability is MUTABLE_WITH_EXCLUSION or IMMUTABLE_WITH_EXCLUSION. Contains filter (pattern string) and filter_type (WILDCARD)."
  type        = any
  default     = null
}
