...
output "lambda_function_arn_static" {
  ...
  value       = local.enabled && var.create_function && !var.create_layer ? "arn:${data.aws_partition.current.partition}:lambda:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:function:${var.name}" : ""
}
...
output "lambda_function_qualified_invoke_arn" {
  ...
  value       = try(aws_lambda_function.this.qualified_invoke_arn, "")
}
...
output "lambda_function_kms_key_arn" {
  ...
  value       = try(aws_lambda_function.this.kms_key_arn, "")
}
