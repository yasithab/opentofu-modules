...
output "lambda_function_arn_static" {
  ...
  value       = local.enabled && var.create_function && !var.create_layer ? "arn:aws:lambda:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:function:${var.function_name}" : ""
}
