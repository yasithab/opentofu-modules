...
Wrapper module that allows creating multiple Lambda functions using a single module block with a `for_each`-driven interface. Each item in the `items` map creates a separate Lambda function instance with its own configuration, while shared settings can be defined once in `defaults`.

## Features

- **Bulk provisioning** - Create multiple Lambda functions from a single module block using a map of items
- **Shared defaults** - Define common configuration once in the `defaults` variable, with per-item overrides
- **Full feature parity** - Passes through all parameters supported by the root Lambda module, including VPC config, IAM roles, layers, environment variables, event source mappings, and more

## Usage

```hcl
module "lambda" {
  source = "git::https://github.com/yasithab/opentofu-modules.git//lambda/wrappers?depth=1&ref=master"
  ...
  items = {
    my_function_1 = {
      ...
      source_path   = "${path.module}/functions/one"
    }
    my_function_2 = {
      function_name = "function-two"
      source_path   = "${path.module}/functions/two"
      memory_size   = 512
    }
  }
}
