...
locals {
  ...
  s3_object_version = var.s3_existing_package != null ? try(var.s3_existing_package.version_id, null) : (var.store_on_s3 ? try(aws_s3_object.lambda_package.version_id, null) : null)
...
resource "aws_lambda_function" "this" {
  ...
  dynamic "tenancy_config" {
    for_each = var.tenancy_config != null ? [var.tenancy_config] : []
  ...
  dynamic "capacity_provider_config" {
    for_each = var.capacity_provider_config != null ? [var.capacity_provider_config] : []

    content {
      dynamic "lambda_managed_instances_capacity_provider_config" {
        for_each = try([capacity_provider_config.value.lambda_managed_instances_capacity_provider_config], [])

        content {
          capacity_provider_arn                     = lambda_managed_instances_capacity_provider_config.value.capacity_provider_arn
          execution_environment_memory_gib_per_vcpu = try(lambda_managed_instances_capacity_provider_config.value.execution_environment_memory_gib_per_vcpu, null)
          per_execution_environment_max_concurrency = try(lambda_managed_instances_capacity_provider_config.value.per_execution_environment_max_concurrency, null)
        }
      }
    }
  }
  ...
  dynamic "durable_config" {
    ...
    content {
      ...
      retention_period  = try(durable_config.value.retention_period, null)
    }
  }
  ...
  tags = merge({ terraform-aws-modules = "lambda" }, local.tags, var.function_tags)
...
resource "aws_lambda_function_url" "this" {
  ...
  qualifier          = var.create_unqualified_alias_lambda_function_url ? null : aws_lambda_function.this.version
...
resource "null_resource" "sam_metadata_aws_lambda_layer_version" {
  triggers = {
    ...
    # resource
    resource_name = "aws_lambda_layer_version.this"
