...
resource "aws_identitystore_group" "sso_groups" {
  for_each          = { for k, v in coalesce(var.sso_groups, {}) : k => v if local.enabled }
...
resource "aws_identitystore_user" "sso_users" {
  for_each          = { for key, user in coalesce(var.sso_users, {}) : user.user_name => user if local.enabled }
  identity_store_id = local.sso_instance_id
  ...
  addresses {
    ...
    formatted = each.value.address_formatted != null ? each.value.address_formatted : join(" ", [
      for part in [
        each.value.street_address,
        each.value.locality,
        each.value.region,
        each.value.postal_code,
        each.value.country,
      ] : part if part != null && part != ""
    ])
...
resource "aws_identitystore_group_membership" "sso_group_membership" {
  for_each          = { for k, v in local.users_and_their_groups : k => v if local.enabled }
  identity_store_id = local.sso_instance_id
...
resource "aws_identitystore_group_membership" "sso_group_membership_existing_google_sso_users" {
  for_each          = { for k, v in local.users_and_their_groups_existing_google_sso_users : k => v if local.enabled }
  identity_store_id = local.sso_instance_id
...
resource "aws_ssoadmin_permission_set" "pset" {
  for_each = { for k, v in var.permission_sets : k => v if local.enabled }
  name     = each.key
...
resource "aws_ssoadmin_managed_policy_attachment" "pset_aws_managed_policy" {
  for_each = { for pset in local.pset_aws_managed_policy_maps : "${pset.pset_name}.${pset.policy_arn}" => pset if local.enabled }
...
resource "aws_ssoadmin_customer_managed_policy_attachment" "pset_customer_managed_policy" {
  for_each = { for pset in local.pset_customer_managed_policy_maps : "${pset.pset_name}.${pset.policy_name}" => pset if local.enabled }
...
resource "aws_ssoadmin_permission_set_inline_policy" "pset_inline_policy" {
  for_each = { for pset in local.pset_inline_policy_maps : pset.pset_name => pset if local.enabled }
...
resource "aws_ssoadmin_permissions_boundary_attachment" "pset_permissions_boundary_aws_managed" {
  for_each = { for pset in local.pset_permissions_boundary_aws_managed_maps : pset.pset_name => pset if local.enabled }
...
resource "aws_ssoadmin_permissions_boundary_attachment" "pset_permissions_boundary_customer_managed" {
  for_each = { for pset in local.pset_permissions_boundary_customer_managed_maps : pset.pset_name => pset if local.enabled }
  ...
  permissions_boundary {
    customer_managed_policy_reference {
      ...
      path = each.value.boundary.customer_managed_policy_reference.path
    }
  }
}
...
resource "aws_ssoadmin_account_assignment" "account_assignment" {
  ...
  for_each = { for k, v in local.principals_and_their_account_assignments : k => v if local.enabled }
  ...
  lifecycle {
    precondition {
      condition     = each.value.account_id != null
      error_message = "Account assignment '${each.key}': '${each.value.account_input}' is neither a 12-digit AWS account ID nor the name of an ACTIVE account in the organization. Check the `account_ids` entries of `account_assignments`."
    }
  }
}
...
resource "aws_ssoadmin_application" "sso_apps" {
  for_each                 = { for k, v in coalesce(var.sso_applications, {}) : k => v if local.enabled }
...
resource "aws_ssoadmin_application_assignment_configuration" "sso_apps_assignments_configs" {
  for_each = {
    ...
    "${assignment_config.app_name}-assignment-config" => assignment_config if local.enabled
  }
...
resource "aws_ssoadmin_application_access_scope" "sso_apps_assignments_access_scope" {
  for_each = {
    ...
    "${app_access_scope.app_name}-${app_access_scope.scope}" => app_access_scope if local.enabled
  }
  application_arn = aws_ssoadmin_application.sso_apps[each.value.app_name].application_arn
  authorized_targets = [
...
resource "aws_ssoadmin_application_assignment" "sso_apps_groups_assignments" {
  for_each = {
    ...
    "${assignment.app_name}-${assignment.group_name}" => assignment if local.enabled
  }
  application_arn = aws_ssoadmin_application.sso_apps[each.value.app_name].application_arn
...
resource "aws_ssoadmin_application_assignment" "sso_apps_users_assignments" {
  for_each = {
    ...
    "${assignment.app_name}-${assignment.user_name}" => assignment if local.enabled
  }
  application_arn = aws_ssoadmin_application.sso_apps[each.value.app_name].application_arn
...
resource "aws_ssoadmin_trusted_token_issuer" "this" {
  for_each = { for k, v in coalesce(var.trusted_token_issuers, {}) : k => v if local.enabled }
...
resource "aws_ssoadmin_instance_access_control_attributes" "sso_access_control_attributes" {
  count        = local.enabled && length(var.sso_instance_access_control_attributes) > 0 ? 1 : 0
  instance_arn = local.ssoadmin_instance_arn
  dynamic "attribute" {
