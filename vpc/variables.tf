...
variable "cidr" {
  ...
  validation {
    condition     = var.cidr == null || can(cidrnetmask(var.cidr))
...
variable "create_multiple_private_route_tables" {
  description = "Indicates whether to create a separate route table for each private subnet. Default: `true`"
  type        = bool
  default     = true
}
...
variable "create_multiple_database_route_tables" {
  description = "Indicates whether to create a separate route table for each database subnet. Default: `true`"
  type        = bool
  default     = true
}
...
variable "nat_gateway_connectivity_type" {
  description = "Connectivity type for the NAT Gateway. Valid values: `public`, `private`. Defaults to `public`"
  type        = string
  default     = "public"

  validation {
    condition     = contains(["public", "private"], var.nat_gateway_connectivity_type)
    error_message = "Valid values for nat_gateway_connectivity_type are 'public' or 'private'."
  }
}
...
variable "nat_gateway_secondary_private_ip_addresses" {
  description = "List of secondary private IPv4 addresses to assign to the NAT Gateway (zonal NAT gateways only)"
  type        = list(string)
  default     = []
}
