# Generated by scripts/generate-offline-tests.py - do not edit by hand.
# Extra mock defaults live in scripts/offline-test-mocks/vpc__vpc-endpoints.hcl.
# Regenerate with: python3 scripts/generate-offline-tests.py vpc/vpc-endpoints

mock_provider "aws" {
  mock_data "aws_caller_identity" {
    defaults = {
      account_id = "123456789012"
      arn        = "arn:aws:iam::123456789012:root"
      user_id    = "AIDAMOCK"
    }
  }

  mock_data "aws_iam_policy_document" {
    defaults = {
      json          = "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":\"*\",\"Resource\":\"*\"}]}"
      minified_json = "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":\"*\",\"Resource\":\"*\"}]}"
    }
  }

  mock_data "aws_partition" {
    defaults = {
      dns_suffix = "amazonaws.com"
      id         = "aws"
      partition  = "aws"
    }
  }

  mock_data "aws_region" {
    defaults = {
      name   = "us-east-1"
      region = "us-east-1"
    }
  }
}

run "plan_enabled" {
  command = plan

  variables {
    vpc_id     = "vpc-12345678"
    subnet_ids = ["subnet-11111111", "subnet-22222222"]

    endpoints = {
      s3 = {
        service         = "s3"
        service_type    = "Gateway"
        route_table_ids = ["rtb-12345678"]
      }
      ssm = {
        service             = "ssm"
        private_dns_enabled = true
      }
    }

    create_security_group      = true
    security_group_name        = "terratest-vpc-endpoints"
    security_group_description = "Terratest VPC endpoints security group"

    security_group_rules = {
      ingress_https = {
        type        = "ingress"
        description = "HTTPS from VPC"
        cidr_ipv4   = "10.99.0.0/16"
      }
      egress_all = {
        type        = "egress"
        ip_protocol = "-1"
        description = "Allow all egress"
        cidr_ipv4   = "0.0.0.0/0"
      }
    }
  }
}

run "plan_disabled" {
  command = plan

  variables {
    vpc_id     = "vpc-12345678"
    subnet_ids = ["subnet-11111111", "subnet-22222222"]

    endpoints = {
      s3 = {
        service         = "s3"
        service_type    = "Gateway"
        route_table_ids = ["rtb-12345678"]
      }
      ssm = {
        service             = "ssm"
        private_dns_enabled = true
      }
    }

    create_security_group      = true
    security_group_name        = "terratest-vpc-endpoints"
    security_group_description = "Terratest VPC endpoints security group"

    security_group_rules = {
      ingress_https = {
        type        = "ingress"
        description = "HTTPS from VPC"
        cidr_ipv4   = "10.99.0.0/16"
      }
      egress_all = {
        type        = "egress"
        ip_protocol = "-1"
        description = "Allow all egress"
        cidr_ipv4   = "0.0.0.0/0"
      }
    }

    enabled = false
  }
}
