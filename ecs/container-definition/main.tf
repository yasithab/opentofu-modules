...
locals {
  ...
  log_configuration = merge(
    { for k, v in {
      ...
      options = {
        ...
        awslogs-group         = try(aws_cloudwatch_log_group.this.name, ""),
  ...
  definition = {
    ...
    cpu                    = var.cpu
    credentialSpecs        = length(var.credential_specs) > 0 ? var.credential_specs : null
    ...
    environmentFiles       = length(var.env_files) > 0 ? var.env_files : null
