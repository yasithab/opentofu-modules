...
variable "cloudwatch_log_group_skip_destroy" {
  description = "Set to true to prevent the log group from being deleted on module destroy. Preserves container logs."
  type        = bool
  default     = false
}
...
variable "credential_specs" {
  description = "A list of ARNs in SSM or Amazon S3 to a credential spec (CredSpec) file that configures the container for Active Directory authentication. Only valid for Windows containers"
  type        = list(string)
  default     = []
}

variable "version_consistency" {
  description = "Whether image tag-to-digest resolution should be enabled for the container image. Valid values are `enabled` or `disabled`"
  type        = string
  default     = null
}
