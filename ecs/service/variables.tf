...
variable "vpc_lattice_configurations" {
  description = "List of VPC Lattice configuration blocks to associate with the service. Each block requires port_name, role_arn, and target_group_arn"
  type = list(object({
    port_name        = string
    role_arn         = string
...
variable "deployment_configuration" {
  description = "Configuration block for deployment configuration (blue/green, canary, linear strategies)"
  type        = any
  default     = {}
}
...
variable "sigint_rollback" {
  description = "Whether to enable rollback of the service when a SIGINT signal is received"
  type        = bool
  default     = null
}
...
variable "service_connect_configuration" {
  ...
  type = object({
    ...
    access_log_configuration = optional(object({
      ...
      include_query_parameters = optional(bool)
    }))
...
variable "skip_destroy" {
  description = "If true, the task definition is not deleted when the service is destroyed"
  type        = bool
  default     = true
}
...
variable "enable_fault_injection" {
  description = "Enables fault injection and allows for fault injection requests to be accepted from the task's containers. Valid values are `true` or `false`"
  type        = bool
  default     = null
}
...
variable "create_infrastructure_iam_role" {
  description = "Determines whether the ECS infrastructure IAM role should be created. Required for managed EBS volumes (volume_configuration) and VPC Lattice"
  type        = bool
  default     = false
}
...
variable "infrastructure_iam_role_name" {
  description = "Name to use on the infrastructure IAM role created"
  type        = string
  default     = null
}

variable "infrastructure_iam_role_use_name_prefix" {
  description = "Determines whether the infrastructure IAM role name is used as a prefix"
  type        = bool
  default     = true
}

variable "infrastructure_iam_role_path" {
  description = "IAM role path for the infrastructure role"
  type        = string
  default     = null
}
...
variable "infrastructure_iam_role_tags" {
  description = "A map of additional tags to add to the infrastructure IAM role created"
  type        = map(string)
  default     = {}
}

variable "infrastructure_iam_role_policies" {
  description = "Map of IAM policy ARNs to attach to the infrastructure IAM role"
  type        = map(string)
  default     = {}
}

################################################################################
# Service - CloudWatch Log Group (service connect)
################################################################################

variable "cloudwatch_log_group_skip_destroy" {
  description = "Set to true to prevent the service-connect log group from being deleted on module destroy"
  type        = bool
  default     = false
}
...
variable "autoscaling_suspended_state" {
  description = "Suspends scaling activities for the autoscaling target. Map of optional boolean keys: dynamic_scaling_in_suspended, dynamic_scaling_out_suspended, scheduled_scaling_suspended"
  type        = any
  default     = null
}
