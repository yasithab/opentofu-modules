...
variable "creation_token" {
  ...
  validation {
    condition     = var.creation_token == null || (length(var.creation_token) >= 1 && length(var.creation_token) <= 64)
    error_message = "creation_token must be between 1 and 64 characters."
  }
}
...
variable "performance_mode" {
  ...
  validation {
    condition     = var.performance_mode == null || contains(["generalPurpose", "maxIO"], var.performance_mode)
...
variable "throughput_mode" {
  ...
  validation {
    condition     = var.throughput_mode == null || contains(["bursting", "elastic", "provisioned"], var.throughput_mode)
...
variable "replication_overwrite_protection" {
  description = "Whether to enable or disable the file system's replication overwrite protection. Valid values: ENABLED or DISABLED. When set to ENABLED, the file system cannot be used as a replication destination."
  type        = string
  default     = null

  validation {
    condition     = var.replication_overwrite_protection == null || contains(["ENABLED", "DISABLED"], var.replication_overwrite_protection)
    error_message = "replication_overwrite_protection must be one of: ENABLED, DISABLED."
  }
}
