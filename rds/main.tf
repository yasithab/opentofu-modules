...
locals {
  ...
  final_snapshot_identifier = var.skip_final_snapshot ? var.final_snapshot_identifier : coalesce(var.final_snapshot_identifier, "${var.name}-final")
...
resource "aws_db_instance" "read_replica" {
  ...
  allocated_storage                     = each.value.allocated_storage != null ? each.value.allocated_storage : var.allocated_storage
  allow_major_version_upgrade           = each.value.allow_major_version_upgrade != null ? each.value.allow_major_version_upgrade : var.allow_major_version_upgrade
  ...
  database_insights_mode                = each.value.database_insights_mode != null ? each.value.database_insights_mode : var.database_insights_mode
  db_subnet_group_name                  = local.db_subnet_group_name
  dedicated_log_volume                  = each.value.dedicated_log_volume != null ? each.value.dedicated_log_volume : var.dedicated_log_volume
  deletion_protection                   = each.value.deletion_protection != null ? each.value.deletion_protection : var.deletion_protection
  enabled_cloudwatch_logs_exports       = each.value.enabled_cloudwatch_logs_exports != null ? each.value.enabled_cloudwatch_logs_exports : var.enabled_cloudwatch_logs_exports
  engine                                = var.engine
  engine_version                        = var.engine_version
  identifier                            = var.use_identifier_prefix ? null : coalesce(each.value.identifier, "${var.name}-${each.key}")
  identifier_prefix                     = var.use_identifier_prefix ? coalesce(each.value.identifier_prefix, "${var.name}-${each.key}-") : null
  ...
  max_allocated_storage                 = each.value.max_allocated_storage != null ? each.value.max_allocated_storage : var.max_allocated_storage
  monitoring_interval                   = each.value.monitoring_interval != null ? each.value.monitoring_interval : var.monitoring_interval
  monitoring_role_arn                   = var.create_monitoring_role ? try(aws_iam_role.rds_enhanced_monitoring.arn, null) : var.monitoring_role_arn
  multi_az                              = each.value.multi_az
  network_type                          = each.value.network_type != null ? each.value.network_type : var.network_type
  option_group_name                     = var.create_db_option_group ? aws_db_option_group.this.name : (each.value.option_group_name != null ? each.value.option_group_name : var.option_group_name)
  parameter_group_name                  = var.create_db_parameter_group ? aws_db_parameter_group.this.id : (each.value.parameter_group_name != null ? each.value.parameter_group_name : var.db_parameter_group_name)
  ...
  replica_mode                          = each.value.replica_mode != null ? each.value.replica_mode : var.replica_mode
  skip_final_snapshot                   = true
  storage_encrypted                     = var.storage_encrypted
  storage_throughput                    = each.value.storage_throughput != null ? each.value.storage_throughput : var.storage_throughput
  storage_type                          = each.value.storage_type != null ? each.value.storage_type : var.storage_type
  vpc_security_group_ids                = compact(concat([try(aws_security_group.this.id, "")], var.vpc_security_group_ids))
