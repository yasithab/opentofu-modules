...
| path | Path in which to create the policy. See IAM Identifiers for more information. | `string` | `"/"` | no |
| policy | The policy document JSON string. Provide exactly one of `policy` or `policy_documents`. | `string` | `null` | no |
| policy\_documents | List of JSON policy document strings to merge into a single policy. Provide exactly one of `policy` or `policy_documents`. | `list(string)` | `[]` | no |
...
| attachment\_count | The number of entities (users, groups, roles) that the policy is attached to. |
