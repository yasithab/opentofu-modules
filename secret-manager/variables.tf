...
variable "secret_resource_policy" {
  description = "A valid JSON document representing a resource policy. When set, this is applied directly to the secret (alternative to create_policy). Note: conflicts with create_policy."
  type        = string
  default     = null
}
...
variable "recovery_window_in_days" {
  ...
  validation {
    ...
    error_message = "recovery_window_in_days must be 0 (force delete) or between 7 and 30 days inclusive."
  }
}
...
variable "secret_string_wo" {
  description = "Write-only text data to encrypt and store in this version of the secret. Requires OpenTofu >= 1.11.0. Mutually exclusive with secret_string."
  type        = string
  default     = null
  ephemeral   = true
}
...
variable "secret_string_wo_version" {
  description = "Increment this value to trigger an update when secret_string_wo changes."
  type        = number
  default     = null
}
...
variable "rotate_immediately" {
  description = "Whether to rotate the secret immediately or wait until the next scheduled rotation window. Defaults to true. Only applies when enable_rotation is true"
  type        = bool
  default     = null
}
