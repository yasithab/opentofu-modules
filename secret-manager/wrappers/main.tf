module "wrapper" {
  ...
  recovery_window_in_days          = try(each.value.recovery_window_in_days, var.defaults.recovery_window_in_days, 30)
  ...
  rotation_lambda_arn              = try(each.value.rotation_lambda_arn, var.defaults.rotation_lambda_arn, null)
