...
variable "allowed_kms_key_arn" {
  ...
  validation {
    condition     = var.allowed_kms_key_arn == null || can(regex("^arn:aws[a-z-]*:kms:", var.allowed_kms_key_arn))
    error_message = "allowed_kms_key_arn must be a valid KMS key ARN (starting with 'arn:aws:kms:') or null."
  }
}
...
variable "s3_bucket_public_access_block_skip_destroy" {
  description = "Indicates whether the bucket public access block should be destroyed on bucket deletion"
  type        = bool
  default     = false
}
...
variable "create_bucket_replication_role" {
  default     = false
  type        = bool
  description = "Create S3 bucket replication role"
}

variable "destination_bucket_name" {
  description = "Name of destination bucket to replicate data"
  type        = string
  default     = null
}
variable "notification_eventbridge" {
  description = "Whether to enable Amazon EventBridge notifications for the S3 bucket"
  type        = bool
  default     = false
}

variable "notification_configuration" {
  description = "JSON-encoded map containing lambda_function, queue, and topic notification configurations. Alternative to individual notification variables."
  type        = string
  default     = null
}
