...
resource "aws_s3_bucket_logging" "this" {
  ...
  dynamic "target_grant" {
    for_each = try(var.logging["target_grants"], [])
...
# trivy:ignore:AVD-AWS-0132 - Encryption algorithm is caller-controlled via var.server_side_encryption_configuration; KMS CMK can be specified by caller
...
resource "aws_s3_bucket_notification" "this" {
  ...
  dynamic "lambda_function" {
    for_each = try(jsondecode(var.notification_configuration)["lambda_function"], var.notification_lambda_functions)
  ...
  dynamic "queue" {
    for_each = try(jsondecode(var.notification_configuration)["queue"], var.notification_queues)
  ...
  dynamic "topic" {
    for_each = try(jsondecode(var.notification_configuration)["topic"], var.notification_topics)
