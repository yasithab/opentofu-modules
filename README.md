...
- `terraform_docs` - regenerate README Requirements/Providers/Inputs/Outputs tables using `.terraform-docs.yml`
...
The Requirements/Providers/Inputs/Outputs tables in every module README are generated by
[terraform-docs](https://terraform-docs.io/) (configured in `.terraform-docs.yml`) and injected
between the `<!-- BEGIN_TF_DOCS -->` / `<!-- END_TF_DOCS -->` markers. Do not edit those tables
by hand — regenerate them instead:

```bash
brew install terraform-docs   # once
task docs                     # regenerate all module READMEs
```

The `terraform_docs` pre-commit hook keeps the tables in sync automatically on commit.

### Creating a New Module

Scaffold a new module directory with the conventional skeleton (`providers.tf`, `variables.tf`,
`main.tf`, `outputs.tf`, `README.md`, `test/test.tfvars`):

```bash
task new-module -- my-module
```

The task refuses to overwrite an existing directory. After adding resources, run
`task format`, `task validate`, and `task docs`.

### Dependency Updates

[Renovate](https://docs.renovatebot.com/) (configured in `renovate.json`) runs weekly and:

- Bumps provider version floors in `providers.tf` files (`rangeStrategy: bump`), grouped into a single PR
- Pins GitHub Actions to commit SHAs and keeps them updated
- Refreshes committed `.terraform.lock.hcl` files via lock file maintenance
...
| `.terraform-docs.yml` | terraform-docs settings for generated README tables |
