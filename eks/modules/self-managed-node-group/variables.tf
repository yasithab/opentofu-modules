...
variable "enable_efa_only" {
  description = "Determines whether to enable EFA only (`true`) or both EFA and EFA-only (`false`) network interfaces. Note: requires vpc-cni version `v1.18.4` or later"
  type        = bool
  default     = true
}
...
variable "availability_zone_distribution" {
  description = "The availability zone distribution settings for the Auto Scaling group"
  type        = any
  default     = {}
}
...
variable "traffic_sources" {
  description = "Set of traffic sources to attach to the Auto Scaling Group. Each object has `identifier` and `type` attributes"
  type = list(object({
    identifier = string
    type       = string
  }))
...
variable "access_entry_kubernetes_groups" {
  description = "List of Kubernetes groups the access entry principal belongs to"
  type        = list(string)
  default     = null
}

variable "access_entry_user_name" {
  description = "The Kubernetes username for the access entry principal"
  type        = string
  default     = null
}
