...
|------|-------------|------|---------|:--------:|
| <a name="input_access_entry_kubernetes_groups"></a> [access\_entry\_kubernetes\_groups](#input\_access\_entry\_kubernetes\_groups) | List of Kubernetes groups the access entry principal belongs to | `list(string)` | `null` | no |
| <a name="input_access_entry_user_name"></a> [access\_entry\_user\_name](#input\_access\_entry\_user\_name) | The Kubernetes username for the access entry principal | `string` | `null` | no |
...
| <a name="input_asg_capacity_reservation_specification"></a> [asg\_capacity\_reservation\_specification](#input\_asg\_capacity\_reservation\_specification) | The capacity reservation specification for the Auto Scaling group | `any` | `{}` | no |
...
| <a name="input_availability_zone_distribution"></a> [availability\_zone\_distribution](#input\_availability\_zone\_distribution) | The availability zone distribution settings for the Auto Scaling group | `any` | `{}` | no |
...
| <a name="input_cpu_options"></a> [cpu\_options](#input\_cpu\_options) | The CPU options for the instance | `any` | `{}` | no |
...
| <a name="input_enable_efa_only"></a> [enable\_efa\_only](#input\_enable\_efa\_only) | Determines whether to enable EFA only (`true`) or both EFA and EFA-only (`false`) network interfaces. Note: requires vpc-cni version `v1.18.4` or later | `bool` | `true` | no |
...
| <a name="input_enabled"></a> [enabled](#input\_enabled) | Determines whether to create self managed node group or not | `bool` | `true` | no |
...
| <a name="input_launch_template_id"></a> [launch\_template\_id](#input\_launch\_template\_id) | The ID of an existing launch template to use. Required when `create_launch_template` = `false` | `string` | `null` | no |
...
| <a name="input_load_balancers"></a> [load\_balancers](#input\_load\_balancers) | A list of elastic load balancer names to add to the autoscaling group names. Only valid for classic load balancers | `list(string)` | `[]` | no |
...
| <a name="input_name"></a> [name](#input\_name) | Name of the Self managed Node Group | `string` | `null` | no |
...
| <a name="input_network_performance_options"></a> [network\_performance\_options](#input\_network\_performance\_options) | The network performance options for the instance. Valid keys are `bandwidth_weighting` (valid values: `default`, `vpc-1`, `ebs-1`) | `map(string)` | `{}` | no |
...
| <a name="input_placement_group_partition_count"></a> [placement\_group\_partition\_count](#input\_placement\_group\_partition\_count) | The number of partitions to create in the placement group. Only valid when `placement_group_strategy` is `partition`. Must be between 1 and 7 | `number` | `null` | no |
| <a name="input_placement_group_spread_level"></a> [placement\_group\_spread\_level](#input\_placement\_group\_spread\_level) | Determines how placement groups spread instances. Can only be used when `placement_group_strategy` is `spread`. Can be `host` or `rack` | `string` | `null` | no |
| <a name="input_placement_group_strategy"></a> [placement\_group\_strategy](#input\_placement\_group\_strategy) | The placement group strategy. Can be `cluster`, `partition`, or `spread` | `string` | `"cluster"` | no |
...
| <a name="input_secondary_interfaces"></a> [secondary\_interfaces](#input\_secondary\_interfaces) | Configuration for secondary network interfaces on the launch template | `list(any)` | `[]` | no |
| <a name="input_security_group_names"></a> [security\_group\_names](#input\_security\_group\_names) | A list of security group names to associate with. (EC2-Classic only) | `list(string)` | `null` | no |
...
| <a name="input_traffic_sources"></a> [traffic\_sources](#input\_traffic\_sources) | Set of traffic sources to attach to the Auto Scaling Group. Each object has `identifier` and `type` attributes | <pre>list(object({<br/>    identifier = string<br/>    type       = string<br/>  }))</pre> | `[]` | no |
