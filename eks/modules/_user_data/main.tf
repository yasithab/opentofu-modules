...
resource "null_resource" "validate_cluster_service_cidr" {
  lifecycle {
    precondition {
      ...
      condition     = var.enabled ? length(local.cluster_service_cidr) > 6 : true
...
locals {
  ...
  # Use `ami_type` first, fall back to `platform` for custom user data templates
  ...
  user_data_type_to_rendered = {
    al2023 = {
      user_data = var.enabled ? try(data.cloudinit_config.al2023_eks_managed_node_group[0].rendered, local.user_data) : ""
    }
    bottlerocket = {
      user_data = var.enabled && local.user_data_type == "bottlerocket" && (var.enable_bootstrap_user_data || var.user_data_template_path != null || var.bootstrap_extra_args != null) ? local.user_data : ""
    }
    linux = {
      user_data = var.enabled ? try(data.cloudinit_config.linux_eks_managed_node_group[0].rendered, local.user_data) : ""
    }
    windows = {
      user_data = var.enabled && local.user_data_type == "windows" && (var.enable_bootstrap_user_data || var.user_data_template_path != null || var.pre_bootstrap_user_data != null) ? local.user_data : ""
    }
  }
}
...
data "cloudinit_config" "linux_eks_managed_node_group" {
  count = var.enabled && local.user_data_type == "linux" && var.is_eks_managed_node_group && !var.enable_bootstrap_user_data && var.pre_bootstrap_user_data != null && var.user_data_template_path == null ? 1 : 0
...
data "cloudinit_config" "al2023_eks_managed_node_group" {
  count = var.enabled && local.user_data_type == "al2023" && length(local.nodeadm_cloudinit) > 0 ? 1 : 0
