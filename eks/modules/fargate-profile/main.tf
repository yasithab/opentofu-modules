...
data "aws_iam_policy_document" "assume_role_policy" {
  ...
  statement {
    ...
    condition {
      ...
      values = [
        "arn:${data.aws_partition.current.partition}:eks:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:fargateprofile/${var.cluster_name}/*",
      ]
    }
  }
}
...
resource "aws_eks_fargate_profile" "this" {
  ...
  pod_execution_role_arn = var.create_iam_role ? aws_iam_role.this.arn : var.iam_role_arn
