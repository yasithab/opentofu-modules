...
data "aws_iam_policy_document" "assume_role" {
  ...
  dynamic "statement" {
    ...
    content {
      ...
      principals {
        type        = "AWS"
        identifiers = [aws_iam_role.intermediate.arn]
      }
    }
  }
...
resource "aws_iam_role_policy_attachment" "this" {
  for_each = { for k, v in merge(
    {
      node                               = try(aws_iam_policy.this.arn, null)
...
resource "aws_rolesanywhere_profile" "this" {
  accept_role_session_name    = var.ira_profile_accept_role_session_name
  duration_seconds            = var.ira_profile_duration_seconds
  enabled                     = var.ira_profile_enabled
  ...
  role_arns                   = [aws_iam_role.intermediate.arn]
  session_policy              = var.ira_profile_session_policy

  tags = local.tags

  lifecycle {
    enabled = local.enable_ira
  }
}
...
resource "aws_rolesanywhere_trust_anchor" "this" {
  enabled = var.ira_trust_anchor_enabled
...
data "aws_iam_policy_document" "intermediate_assume_role" {
  ...
  statement {
    ...
    condition {
      ...
      values   = concat(var.trust_anchor_arns, try([aws_rolesanywhere_trust_anchor.this.arn], []))
    }
  }
}
...
resource "aws_iam_role_policy_attachment" "intermediate" {
  for_each = { for k, v in merge(
    {
      intermediate                       = try(aws_iam_policy.intermediate.arn, null)
      AmazonEC2ContainerRegistryPullOnly = "arn:${local.partition}:iam::aws:policy/AmazonEC2ContainerRegistryPullOnly"
    },
