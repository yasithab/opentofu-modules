...
variable "pod_identity_target_role_arn" {
...
variable "pod_identity_disable_session_tags" {
  description = "Whether to disable automatic session tags for the Pod Identity association"
  type        = bool
  default     = null
}
...
variable "queue_visibility_timeout_seconds" {
  description = "Visibility timeout for the queue. An integer from 0 to 43200 (12 hours). Defaults to 30"
  type        = number
  default     = null
}

variable "queue_delay_seconds" {
  description = "Time in seconds that the delivery of all messages in the queue will be delayed. An integer from 0 to 900. Defaults to 0"
  type        = number
  default     = null
}

variable "queue_receive_wait_time_seconds" {
  description = "Time for which a ReceiveMessage call will wait for a message to arrive (long polling) before returning. An integer from 0 to 20. Defaults to 0"
  type        = number
  default     = null
}

variable "queue_max_message_size" {
  description = "Limit of how many bytes a message can contain before Amazon SQS rejects it. An integer from 1024 bytes (1 KiB) up to 262144 bytes (256 KiB). Defaults to 262144"
  type        = number
  default     = null
}
...
variable "event_bus_name" {
  description = "The name or ARN of the event bus to associate with EventBridge rules and targets. Defaults to the default event bus"
  type        = string
  default     = null
}
