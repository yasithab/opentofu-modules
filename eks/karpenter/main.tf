...
resource "aws_eks_pod_identity_association" "karpenter" {
  ...
  target_role_arn      = var.pod_identity_target_role_arn
  disable_session_tags = var.pod_identity_disable_session_tags

  tags = local.tags

  lifecycle {
    enabled = local.create_iam_role && var.enable_pod_identity && var.create_pod_identity_association
  }
}
...
resource "aws_sqs_queue" "this" {
  ...
  sqs_managed_sse_enabled           = var.queue_kms_master_key_id == null ? var.queue_managed_sse_enabled : null
  ...
  lifecycle {
    enabled = local.enable_spot_termination
  }
}
...
resource "aws_cloudwatch_event_rule" "this" {
  ...
  tags = merge({ "ClusterName" : var.cluster_name }, local.tags)
}
