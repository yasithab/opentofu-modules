...
resource "aws_kinesis_firehose_delivery_stream" "this" {
  ...
  dynamic "msk_source_configuration" {
    ...
    content {
      ...
      read_from_timestamp = var.msk_source_read_from_timestamp
    }
  }
  ...
  dynamic "splunk_configuration" {
    ...
    content {
      ...
      buffering_size             = var.splunk_buffering_size
      buffering_interval         = var.splunk_buffering_interval
  ...
  dynamic "elasticsearch_configuration" {
    ...
    content {
      ...
      cluster_endpoint      = var.elasticsearch_cluster_endpoint
      role_arn              = local.firehose_role_arn
  ...
  dynamic "opensearch_configuration" {
    ...
    content {
      ...
      cluster_endpoint      = var.opensearch_cluster_endpoint
      role_arn              = local.firehose_role_arn
  ...
  dynamic "iceberg_configuration" {
    ...
    content {
      ...
      append_only = try(var.iceberg_append_only, null)

      dynamic "destination_table_configuration" {
        for_each = var.iceberg_destination_tables
        ...
        content {
          ...
          s3_error_output_prefix = try(destination_table_configuration.value.s3_error_output_prefix, null)
          unique_keys            = try(destination_table_configuration.value.unique_keys, null)
        }
      }
...
resource "aws_cloudwatch_log_group" "log" {
  ...
  lifecycle {
    enabled = local.create_destination_logs || local.create_backup_logs
  }
}
...
resource "aws_cloudwatch_log_stream" "backup" {
  ...
  lifecycle {
    enabled = local.create_backup_logs
  }
}
...
resource "aws_cloudwatch_log_stream" "destination" {
  ...
  lifecycle {
    enabled = local.create_destination_logs
  }
}
...
resource "aws_security_group" "firehose" {
  ...
  tags = merge(local.tags, var.vpc_security_group_tags)

  lifecycle {
    enabled = local.search_destination_vpc_create_firehose_sg
  }
}
...
resource "aws_security_group" "destination" {
  ...
  lifecycle {
    enabled = local.vpc_create_destination_group
  }
}
...
# Configure rules on existing firehose SGs (ingress self when same-as-destination, egress when separate)
resource "aws_vpc_security_group_ingress_rule" "firehose_existing_self" {
  for_each = local.search_destination_vpc_configure_existing_firehose_sg && var.vpc_security_group_same_as_destination ? toset(var.vpc_security_group_firehose_ids) : toset([])
...
resource "aws_vpc_security_group_egress_rule" "firehose_existing_egress" {
  for_each = local.search_destination_vpc_configure_existing_firehose_sg && !var.vpc_security_group_same_as_destination ? toset(flatten([for security_group in var.vpc_security_group_firehose_ids : [for destination_sg in local.search_destination_vpc_destination_sgs : "${security_group}_${destination_sg}"]])) : toset([])

  security_group_id            = split("_", each.value)[0]
  ip_protocol                  = "tcp"
  from_port                    = 443
  to_port                      = 443
  referenced_security_group_id = split("_", each.value)[1]
  description                  = "Allow Outbound HTTPS Traffic"

  tags = local.tags
}

# Configure rules on existing destination SGs
resource "aws_vpc_security_group_ingress_rule" "destination_existing_from_sg" {
  for_each = local.vpc_configure_destination_group && local.is_search_destination ? toset(flatten([for security_group in var.vpc_security_group_destination_ids : [for destination_sg in local.search_destination_vpc_firehose_sgs : "${security_group}_${destination_sg}"]])) : toset([])
