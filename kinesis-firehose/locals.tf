locals {
  ...
  # Data Transformation
  enable_processing = var.enable_lambda_transform || var.enable_dynamic_partitioning || var.enable_cloudwatch_logs_decompression || var.append_delimiter_to_record
  ...
  append_delimiter_processor = var.append_delimiter_to_record ? {
  ...
  # S3 Backup
  use_backup_vars_in_s3_configuration = contains(["elasticsearch", "opensearch", "opensearchserverless", "splunk", "http_endpoint", "snowflake", "iceberg"], local.destination) ? true : false
