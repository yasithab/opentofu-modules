...
resource "aws_iam_policy" "kinesis" {
  ...
  lifecycle {
    enabled = local.add_kinesis_source_policy
  }
}
...
resource "aws_iam_policy" "lambda" {
  ...
  lifecycle {
    enabled = local.add_lambda_policy
  }
}
...
resource "aws_iam_policy" "s3_kms" {
  ...
  lifecycle {
    enabled = local.add_s3_kms_policy
  }
}
...
resource "aws_iam_policy" "elasticsearch" {
  ...
  lifecycle {
    enabled = local.add_elasticsearch_policy
  }
}
...
resource "aws_iam_policy" "opensearch" {
  ...
  lifecycle {
    enabled = local.add_opensearch_policy
  }
}
...
resource "aws_iam_service_linked_role" "opensearch" {
  ...
  lifecycle {
    enabled = contains(["elasticsearch", "opensearch"], local.destination) && var.enable_vpc && var.opensearch_vpc_create_service_linked_role
  }
}
...
resource "aws_iam_policy" "opensearchserverless" {
  ...
  lifecycle {
    enabled = local.add_opensearchserverless_policy
  }
}

resource "aws_iam_role_policy_attachment" "opensearchserverless" {
  role       = aws_iam_role.firehose.name
  policy_arn = aws_iam_policy.opensearchserverless.arn
...
resource "aws_iam_service_linked_role" "opensearchserverless" {
  ...
  lifecycle {
    enabled = local.destination == "opensearchserverless" && var.opensearch_vpc_create_service_linked_role
  }
}
...
resource "aws_iam_role" "application" {
  ...
  tags = merge(local.tags, var.application_role_tags)

  lifecycle {
    enabled = var.create_application_role
  }
}
...
resource "aws_iam_policy" "application" {
  ...
  lifecycle {
    enabled = local.create_application_role_policy
  }
}

resource "aws_iam_role_policy_attachment" "application" {
  role       = var.create_application_role ? aws_iam_role.application.name : var.application_role_name
  policy_arn = aws_iam_policy.application.arn

  lifecycle {
    enabled = (var.create_application_role || var.configure_existing_application_role) && local.create_application_role_policy
  }
}
...
resource "aws_iam_policy" "secretsmanager" {
  ...
  lifecycle {
    enabled = local.add_secretsmanager_policy
  }
}
...
resource "aws_iam_policy" "secretsmanager_cmk_encryption" {
  ...
  lifecycle {
    enabled = local.add_secretsmanager_decrypt_policy
  }
}

resource "aws_iam_role_policy_attachment" "secretsmanager_cmk_encryption" {
  role       = aws_iam_role.firehose.name
  policy_arn = aws_iam_policy.secretsmanager_cmk_encryption.arn
