...
variable "cloudwatch_log_group_deletion_protection_enabled" {
  description = "Set to true to protect the log group from accidental deletion."
  type        = bool
  default     = true
}
...
variable "s3_file_extension" {
  description = "The file extension to override the default file extension (e.g., .json, .csv). Used in the extended_s3 destination."
  type        = string
  default     = null
}

variable "s3_custom_time_zone" {
  description = "The time zone you prefer. UTC and all the IANA time zones are supported. Used in the extended_s3 destination."
  type        = string
  default     = null
}
...
variable "msk_source_read_from_timestamp" {
  description = "The start date and time in UTC for the offset position within the MSK topic from where Firehose begins to read. By default, this is set to timestamp when Firehose becomes Active. Set to Epoch (1970-01-01T00:00:00Z) for Earliest start position."
  type        = string
  default     = null
}
...
variable "elasticsearch_domain_arn" {
  description = "The ARN of the Amazon ES domain. The pattern needs to be arn:.*. Conflicts with elasticsearch_cluster_endpoint."
  type        = string
  default     = null
}

variable "elasticsearch_cluster_endpoint" {
  description = "The endpoint to use when communicating with the Elasticsearch cluster. Conflicts with elasticsearch_domain_arn."
  type        = string
  default     = null
}
...
variable "opensearch_domain_arn" {
  description = "The ARN of the Amazon Opensearch domain. The pattern needs to be arn:.*. Conflicts with opensearch_cluster_endpoint."
  type        = string
  default     = null
}

variable "opensearch_cluster_endpoint" {
  description = "The endpoint to use when communicating with the OpenSearch cluster. Conflicts with opensearch_domain_arn."
  type        = string
  default     = null
}
...
variable "splunk_buffering_size" {
  description = "Buffer incoming data to the specified size, in MBs, before delivering it to the Splunk destination. Valid values between 1 and 5."
  type        = number
  default     = null
}

variable "splunk_buffering_interval" {
  description = "Buffer incoming data for the specified period of time, in seconds, before delivering it to the Splunk destination. Valid values between 0 and 60."
  type        = number
  default     = null
}
...
variable "iceberg_catalog_arn" {
  description = "The ARN of the AWS Glue catalog for the Iceberg destination"
  type        = string
  default     = null
}

variable "iceberg_destination_tables" {
  description = "List of destination table configuration blocks for the Iceberg destination. Each block requires database_name and table_name."
  type        = any
  default     = []
}

variable "iceberg_retry_duration" {
  description = "The period of time, in seconds, during which Firehose retries to deliver data to the Iceberg destination after a failure. Valid values between 0 and 7200."
  type        = number
  default     = 300
  validation {
    error_message = "Valid values: minimum 0 seconds, maximum 7200 seconds."
    condition     = var.iceberg_retry_duration >= 0 && var.iceberg_retry_duration <= 7200
  }
}

variable "iceberg_append_only" {
  description = "Set to true to enable append-only mode for the Iceberg destination (v6.8+). When enabled, Firehose delivers data as inserts only without updating existing records."
  type        = bool
  default     = null
}
