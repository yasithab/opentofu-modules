...
output "oidc_issuer" {
  ...
  value       = try("https://cognito-idp.${data.aws_region.current.name}.amazonaws.com/${aws_cognito_user_pool.this.id}", null)
}
...
output "domain" {
  ...
  value = try(
    var.custom_domain != "" ? var.custom_domain : "${var.domain}.auth.${data.aws_region.current.name}.amazoncognito.com",
    null,
  )
}
...
output "hosted_ui_url" {
  ...
  value = try(
    var.custom_domain != "" ? "https://${var.custom_domain}" : "https://${var.domain}.auth.${data.aws_region.current.name}.amazoncognito.com",
    null,
  )
}
