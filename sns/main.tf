...
data "aws_iam_policy_document" "this" {
  count = local.enabled && var.create_topic_policy ? 1 : 0
...
resource "aws_sns_topic_policy" "this" {
  ...
  lifecycle {
    enabled = local.enabled && var.create_topic_policy
  }
}
...
resource "aws_sns_topic_data_protection_policy" "this" {
  ...
  lifecycle {
    enabled = local.enabled && var.data_protection_policy != null && !var.fifo_topic
  }
}
