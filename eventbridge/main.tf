...
locals {
  ...
  create_log_delivery = var.enabled && var.create_log_delivery
...
resource "aws_cloudwatch_event_bus" "this" {
  ...
  lifecycle {
    enabled = var.enabled && var.create_bus
  }
}
...
resource "aws_cloudwatch_log_delivery" "this" {
  ...
  delivery_source_name     = var.create_log_delivery_source ? aws_cloudwatch_log_delivery_source.this.name : each.value.source_name
...
resource "aws_schemas_discoverer" "this" {
  ...
  source_arn  = var.create_bus ? aws_cloudwatch_event_bus.this.arn : data.aws_cloudwatch_event_bus.this[0].arn
  description = var.schemas_discoverer_description

  tags = local.tags

  lifecycle {
    enabled = var.enabled && var.create_schemas_discoverer
  }
}

resource "aws_cloudwatch_event_rule" "this" {
  for_each = { for k, v in local.eventbridge_rules : v.name => v if var.enabled && var.create_rules }
...
resource "aws_cloudwatch_event_target" "this" {
  for_each = { for k, v in local.eventbridge_targets : v.name => v if var.enabled && var.create_targets }
  ...
  role_arn = can(length(each.value.attach_role_arn) > 0) ? each.value.attach_role_arn : (try(each.value.attach_role_arn, null) == true ? aws_iam_role.eventbridge.arn : null)
  ...
  dynamic "redshift_target" {
    for_each = lookup(each.value, "redshift_target", null) != null ? [
      each.value.redshift_target
    ] : []
  ...
  dynamic "sagemaker_pipeline_target" {
    for_each = lookup(each.value, "sagemaker_pipeline_target", null) != null ? [
      each.value.sagemaker_pipeline_target
    ] : []

    content {
      dynamic "pipeline_parameter_list" {
        for_each = try(sagemaker_pipeline_target.value.pipeline_parameter_list, [])
...
resource "aws_cloudwatch_event_archive" "this" {
  for_each = var.enabled && var.create_archives ? var.archives : {}

  region = var.region

  name             = lookup(each.value, "name", each.key)
  event_source_arn = try(each.value["event_source_arn"], var.create_bus ? aws_cloudwatch_event_bus.this.arn : data.aws_cloudwatch_event_bus.this[0].arn)
...
resource "aws_cloudwatch_event_permission" "this" {
  for_each = var.enabled && var.create_permissions ? var.permissions : {}
...
resource "aws_cloudwatch_event_api_destination" "this" {
  for_each = { for k, v in local.eventbridge_api_destinations : v.name => v if var.enabled && var.create_api_destinations }
...
resource "aws_scheduler_schedule_group" "this" {
  for_each = { for k, v in local.eventbridge_schedule_groups : k => v if var.enabled && var.create_schedule_groups }
...
resource "aws_scheduler_schedule" "this" {
  for_each = { for k, v in local.eventbridge_schedules : v.name => v if var.enabled && var.create_schedules }
  ...
  action_after_completion = lookup(each.value, "action_after_completion", null)
  ...
  target {
    arn      = each.value.arn
    role_arn = can(length(each.value.role_arn) > 0) ? each.value.role_arn : aws_iam_role.eventbridge.arn
  ...
  dynamic "source_parameters" {
    ...
    content {
      ...
      dynamic "activemq_broker_parameters" {
        for_each = try([source_parameters.value.activemq_broker_parameters], [])
        ...
        content {
          ...
          credentials {
            basic_auth = activemq_broker_parameters.value.basic_auth
          }
        }
      }
  ...
  dynamic "target_parameters" {
    ...
    content {
      ...
      dynamic "batch_job_parameters" {
        for_each = try([target_parameters.value.batch_job_parameters], [])
        ...
        content {
          ...
          dynamic "array_properties" {
            for_each = try([batch_job_parameters.value.array_properties], [])
            content {
          ...
          dynamic "container_overrides" {
            for_each = try([batch_job_parameters.value.container_overrides], [])
            content {
              ...
              dynamic "resource_requirement" {
                for_each = try(container_overrides.value.resource_requirement, [])
                content {
          ...
          dynamic "retry_strategy" {
            for_each = try([batch_job_parameters.value.retry_strategy], [])
            content {
      ...
      dynamic "redshift_data_parameters" {
        for_each = try([target_parameters.value.redshift_data_parameters], [])
