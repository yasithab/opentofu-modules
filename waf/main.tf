...
locals {
  ...
  # ignore_changes = [rule] is created instead of the plain one.
  rule_group_associations_enabled = length(var.rule_group_associations) > 0

  # ARN of whichever Web ACL variant is active.
  web_acl_arn = try(aws_wafv2_web_acl.this.arn, aws_wafv2_web_acl.rule_group_associated.arn, "")
}
...
resource "aws_wafv2_rule_group" "this" {
  ...
  rules_json = each.value.rules_json

  # Structured rules (only when rules_json is null)
  dynamic "rule" {
    for_each = each.value.rules_json == null ? each.value.rules : []
    content {
      ...
      # rule.value is dynamically typed (rules = any): try() covers a missing
      # per-rule visibility_config; coalesce covers explicit nulls and falls
      # back to the rule-group-level settings.
      visibility_config {
        cloudwatch_metrics_enabled = coalesce(try(rule.value.visibility_config.cloudwatch_metrics_enabled, null), each.value.cloudwatch_metrics_enabled, true)
        metric_name                = coalesce(try(rule.value.visibility_config.metric_name, null), "${each.key}-${rule.value.name}")
        sampled_requests_enabled   = coalesce(try(rule.value.visibility_config.sampled_requests_enabled, null), each.value.sampled_requests_enabled, true)
      }
    }
  }
  ...
  # each.value is fully typed with optional() defaults, so try() would be dead
  # here (typed optional = null, not missing) - coalesce handles explicit nulls.
  visibility_config {
    cloudwatch_metrics_enabled = coalesce(each.value.cloudwatch_metrics_enabled, true)
    metric_name                = coalesce(each.value.metric_name, each.key)
    sampled_requests_enabled   = coalesce(each.value.sampled_requests_enabled, true)
  }
...
resource "aws_wafv2_web_acl" "this" {
  ...
  dynamic "rule" {
    ...
    content {
      ...
      # rule.value is dynamically typed (var.rules = any): try() covers a
      # missing per-rule visibility_config; coalesce covers explicit nulls.
      visibility_config {
        cloudwatch_metrics_enabled = coalesce(try(rule.value.visibility_config.cloudwatch_metrics_enabled, null), var.visibility_config.cloudwatch_metrics_enabled, true)
        metric_name                = coalesce(try(rule.value.visibility_config.metric_name, null), "${local.name}-${rule.value.name}")
        sampled_requests_enabled   = coalesce(try(rule.value.visibility_config.sampled_requests_enabled, null), var.visibility_config.sampled_requests_enabled, true)
      }
  ...
  # --------------------------------------------------
  visibility_config {
    # var.visibility_config is fully typed with optional() defaults, so try()
    # would be dead here (typed optional = null, not missing) - coalesce
    # handles explicit nulls instead.
    cloudwatch_metrics_enabled = coalesce(var.visibility_config.cloudwatch_metrics_enabled, true)
    metric_name                = coalesce(var.visibility_config.metric_name, local.name)
    sampled_requests_enabled   = coalesce(var.visibility_config.sampled_requests_enabled, true)
  }
  ...
  lifecycle {
    enabled = local.enabled && !local.rule_group_associations_enabled
  }
}

###################################################
# Web ACL (variant used when rule_group_associations is non-empty)
#
# Identical to aws_wafv2_web_acl.this except that the rule attribute is in
# lifecycle ignore_changes. OpenTofu cannot make ignore_changes conditional,
# so two resources are maintained (same pattern as the dynamodb module) and
# exactly one is enabled based on local.rule_group_associations_enabled.
###################################################
resource "aws_wafv2_web_acl" "rule_group_associated" {
  name        = local.name
  scope       = var.scope
  description = var.description
  ...
  lifecycle {
    enabled = local.enabled && local.rule_group_associations_enabled
    # Rules attached via aws_wafv2_web_acl_rule_group_association are managed
    # out-of-band from the inline rule blocks and must be ignored here to
    # prevent OpenTofu from overwriting externally-managed rule entries.
    ignore_changes = [rule]
  }
}
...
resource "aws_wafv2_web_acl_rule_group_association" "this" {
  ...
  # each.value is dynamically typed (var.rule_group_associations = any): try()
  # covers a missing visibility_config; coalesce covers explicit nulls.
  visibility_config {
    cloudwatch_metrics_enabled = coalesce(try(each.value.visibility_config.cloudwatch_metrics_enabled, null), var.visibility_config.cloudwatch_metrics_enabled, true)
    metric_name                = coalesce(try(each.value.visibility_config.metric_name, null), "${local.name}-${each.key}")
    sampled_requests_enabled   = coalesce(try(each.value.visibility_config.sampled_requests_enabled, null), var.visibility_config.sampled_requests_enabled, true)
  }
}
    ...
    content {
      ...
      dynamic "filter" {
        ...
        content {
          ...
          requirement = upper(filter.value.requirement)
