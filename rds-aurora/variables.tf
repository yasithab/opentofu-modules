...
variable "db_system_id" {
  description = "For use with RDS Custom. The ID of the DB system for the DB cluster"
  type        = string
  default     = null
}
...
variable "master_password_wo" {
  description = "Write-only password for the master DB user. Never stored in Terraform state. Required unless `manage_master_user_password` is set to `true` or unless `snapshot_identifier` or `replication_source_identifier` is provided or unless a `global_cluster_identifier` is provided when the cluster is the secondary cluster of a global database"
  type        = string
  default     = null
  ephemeral   = true
}
...
variable "instance_force_destroy" {
  description = "Destroys a read replica cluster instance before deletion. This can be used to force deletion of a read replica without first stopping replication"
  type        = bool
  default     = null
}
