...
variable "certificate_export" {
  description = "Whether the certificate can be exported. Valid values: ENABLED, DISABLED"
  type        = string
  default     = null
}
...
variable "validation_method" {
  description = "Which method to use for validation. DNS, EMAIL or NONE are valid. NONE is used for certificates that were imported into ACM. This parameter must not be set for certificates that were imported into ACM and then into Terraform."
  type        = string
  default     = null

  validation {
    condition     = var.validation_method == null || contains(["DNS", "EMAIL", "NONE"], var.validation_method)
    error_message = "This variable is optional. Valid values are DNS, EMAIL, NONE, or null."
  }
}

variable "early_renewal_duration" {
  description = "Amount of time to start automatic renewal process before expiration. Represented in RFC3339 duration format (e.g. 2160h = 90 days)."
  type        = string
  default     = null
}
...
variable "private_key" {
  description = "Certificate's PEM-formatted private key. Required when importing an existing certificate."
  type        = string
  default     = null
  sensitive   = true
}
...
variable "certificate_body" {
  description = "Certificate's PEM-formatted public key. Required when importing an existing certificate."
  type        = string
  default     = null
}

variable "certificate_chain" {
  description = "Certificate's PEM-formatted chain. Optional when importing an existing certificate."
  type        = string
  default     = null
}
