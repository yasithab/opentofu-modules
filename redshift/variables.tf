...
variable "aqua_configuration_status" {
  description = "The value represents how the cluster is configured to use AQUA (Advanced Query Accelerator) after the cluster is restored. Possible values are `enabled`, `disabled`, and `auto`. Requires `node_type` to be RA3"
  type        = string
  default     = null
}
...
variable "master_password" {
  description = "Password for the master DB user. (Required unless a `snapshot_identifier` is provided). Must contain at least 8 chars, one uppercase letter, one lowercase letter, and one number. Conflicts with `use_master_password_wo`"
  type        = string
  default     = null
  sensitive   = true
}

variable "use_master_password_wo" {
  description = "Whether to use the write-only `master_password_wo` attribute instead of `master_password`. When true, the password is never stored in Terraform state. Requires OpenTofu >= 1.11.0"
  type        = bool
  default     = false
}

variable "master_password_wo_version" {
  description = "Version counter for `master_password_wo`. Increment to trigger a password rotation when `use_master_password_wo` is true"
  type        = number
  default     = 1
}
