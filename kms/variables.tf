...
variable "xks_key_id" {
  description = "ID of the external key that serves as key material inside AWS KMS for an XKS key. Required when creating an XKS key (customer_master_key_spec = SYMMETRIC_DEFAULT and custom_key_store_id points to an external key store)"
  type        = string
  default     = null
}
